	...
	while (window.pollEvent(event)) {
		...
		if (mainPerson.isDeath == false) {
			...
			if (event.type == Event::KeyPressed) {
				if (Keyboard::isKeyPressed(Keyboard::C)) 
				{
					if (mainPerson.isInUseField(pos.x, pos.y, true)) {
						mainPerson.actionAlternate(game.world, pos);// ИСПРАВЬ
					}
				} 
				...
				{
					if (mainPerson.isInUseField(pos.x, pos.y, true)) {
						mainPerson.actionMain(game.world, pos);// ИСПРАВЬ
					}
				} 
			...
			if (event.type == Event::MouseButtonPressed) {
				...
				mainPerson.useItem(game.world, deltaTime, event, pos);// ИСПРАВЬ
...
void render(Game & game)
{
	...
	int lowBorder = mainPerson.currentLevelFloor - 1;
	if (lowBorder <= 0)
		lowBorder = 0;
	int topBorder = mainPerson.currentLevelFloor + 2;
	if (topBorder > HEIGHT_MAP - 1)
		topBorder = HEIGHT_MAP - 1;

	while (l >= lowBorder && l <= topBorder)
	{

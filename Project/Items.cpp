...
void initializeItems(vector<Item> &items, TypesItem *typesItem, Item &emptyItem)
{
	...
	for (size_t i = 0; i < 3; i++) {
		...
		addItem->setType(typesItem->typesItem[idItem::rawMeatWolfItem]);// ИСПРАВЬ
		addItem->setPosition(8, 3, 1);
		items.push_back(*addItem);

		addItem->setType(typesItem->typesItem[idItem::roastMeatWolfItem]);// ИСПРАВЬ
		addItem->setPosition(8, 4, 1);
		items.push_back(*addItem);

		addItem->setType(typesItem->typesItem[idItem::seadlingOakItem]);// ИСПРАВЬ
		addItem->setPosition(9, 2, 1);
		items.push_back(*addItem);

		addItem->setType(typesItem->typesItem[idItem::appleItem]);// ИСПРАВЬ
